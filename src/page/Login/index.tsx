    ...
    <div className={styles.body}>
      <div className="grid justify-items-center mt-24">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-xl flex flex-col px-20 p-2 ">
          ...
          <input type="email" {...register('email')} id="email" className="rounded pl-1" name='email' placeholder='Digite seu email' value={email} onChange={(e) => setEmail(e.target.value)} />
          <p className='text-slate-300'>{errors.email?.message}</p>

          <label className='p-1 text-slate-300'>Senha:</label>

          <input type="password"  {...register('password')} id="password" className="rounded pl-1" name='password' placeholder='*****' value={password} onChange={(e) => setPassword(e.target.value)} />
